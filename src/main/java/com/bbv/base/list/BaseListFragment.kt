 ...
 * 列表 Fragment 基类
 *
 * 基于 SmartRefreshLayout + RecyclerView + StateView 的列表页。
 ...
 *     BaseListFragment<FragmentOrderListBinding, OrderListViewModel, OrderBean, ItemOrderListBinding>() {
 *
 ...
 * ```
 */
abstract class BaseListFragment<VB : ViewBinding, VM : BaseListViewModel<T>, T, VDB : ViewDataBinding> :
    BaseVMFragment<VB, VM>() {
    ...
    override fun onFragmentCreated(view: View, savedInstanceState: Bundle?) {
        ...
        mRefreshLayout = view.findViewById(R.id.refreshLayoutView)
    ...
    private fun observeViewModel() {
        ...
        mViewModel.liveDataList.observe(viewLifecycleOwner) { list ->
            mAdapter.setNewList(list.toMutableList())
        }

        // 刷新信号 → 结束下拉动画 + 重置无更多数据状态
        mViewModel.mRefresh.observe(viewLifecycleOwner) {
        ...
        // 注意：mPageState 由父类 BaseVMFragment 通过 dispatchPageState 自动观察
    }
